...
func startHttpServer() {
	...
	log.Println("👉 Starting HTTP Server (Port 1339)")

	router := mux.NewRouter()

	log.Println("   👉 /calc/{mode}/{n}/{p}/{P}")
	router.HandleFunc("/calc/{mode}/{n}/{p}/{P}", func(writer http.ResponseWriter, request *http.Request) {
		...
		var resv *float64
		if mode == "ge" {
			res, resv = findUpperBoundGe(writer, n, p, P)
		} else if mode == "le" {
			res, resv = findUpperBoundLe(writer, n, p, P)
		}
		...
		if res == nil {
			...
			v := *res
			_, _ = fmt.Fprintln(writer, "😊 Result:", v)
			_, _ = fmt.Fprintln(writer, "")

			if resv == nil {
				_, _ = fmt.Fprintln(writer, "😡 resv was empty.")
				return
			}

			f := F(n, p, v-1)
			_, _ = fmt.Fprint(writer, " F(k = ", v-1, ") = ", f)
			if f <= P {
				_, _ = fmt.Fprintln(writer, " <= P")
			} else {
				_, _ = fmt.Fprintln(writer, " > P")
			}
			_, _ = fmt.Fprint(writer, "*F(k = ", v, ") = ", *resv)
			if *resv <= P {
				_, _ = fmt.Fprintln(writer, " <= P")
			} else {
				_, _ = fmt.Fprintln(writer, " > P")
...
<script>
	...
	log.Println("👾 Binding in port 1339...")
